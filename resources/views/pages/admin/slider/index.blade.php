...
@section('content_admin')
    ...
    <div class="container-fluid" id="container-wrapper">
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">Tất cả thanh trượt</h1>
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('home_admin') }}">Trang chủ</a></li>
                <li class="breadcrumb-item active" aria-current="page">Tất cả thanh trượt</li>
            </ol>
        </div>
            ...
            <div class="col-lg-12">
                <div class="card mb-4">
                    <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                        <h6 class="m-0 font-weight-bold text-primary">Tất cả thanh trượt</h6>
                        <a href="{{ route('add_slider') }}" title="Thêm mới" class="btn btn-success">
                                ...
                                @if ($all)
                                    @foreach ($all as $key => $item)
                                        <tr>
                                            <td>{{ ++$key }}</td>
                                            @php
                                                $anh = "home/img_slider/".$item->hinh_anh
                                            @endphp
                                            ...
                                            <td>
                                                <form method="POST" action="{{ route('delete_slider') }}">
                                                    @csrf
                                                <a style="float: left" href="{{ route('edit_slider', ['id' => $item->ma]) }}" title="Cập nhật & Xem chi tiết" class="btn btn-info mr-1">
