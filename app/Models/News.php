...
class News extends Model
{
    ...
    public function _get_all()
    {
        return $this->orderBy('ma', 'desc')
            ->select(
                'tin_tuc.*',
                DB::raw('(DATE_FORMAT(tin_tuc.ngay_tao,"%H:%i %d-%m-%Y")) as ngay_tao'),
                DB::raw('(DATE_FORMAT(tin_tuc.ngay_cap_nhat,"%H:%i %d-%m-%Y")) as ngay_cap_nhat')
            )->get();
    }

    public function _get_all_and_check_status()
    {
        return $this->orderBy('ma', 'desc')
            ->where('trang_thai', 1)
            ->select(
    ...
    public function _insert($insert, $request)
    {
        ...
        try {
            ...
            if ($get_image) {
                ...
                $get_image->move('home/img_tin_tuc', $new_image);
    ...
    public function _delete($id)
    {
        try {
            ...
            if ($delete->hinh_anh == "") {
                ...
                $img = 'home/img_tin_tuc/' . $delete->hinh_anh;
                if (file_exists($img)) {
                    unlink($img);
                }
            }
    ...
    public function _update($data, $id, $request)
    {
        ...
        try {
            ...
            if ($get_image) {
                ...
                $pro = $this::find($id);
                if (!$pro->hinh_anh == "") {
                    $img = 'home/img_tin_tuc/' . $pro->hinh_anh;
